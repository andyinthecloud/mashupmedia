...
public class StreamingController {
	...
	// @RequestMapping(value = "/media/{mediaItemId}/*", method =
	// RequestMethod.GET)
	// public ModelAndView getMediaStream(@PathVariable("mediaItemId") Long
	// mediaItemId, Model model) throws Exception {
	// final MediaItem mediaItem = mediaManager.getMediaItem(mediaItemId);
	// String format = mediaItem.getFormat();
	// final String contentType = WebHelper.getContentType(format,
	// FormatContentType.MIME);
	//
	...
	// response.setHeader("Content-Range", "bytes 0-" + (fileSize - 1) + "/" +
	// fileSize );
...
//		final boolean content = true;
		...
		ModelAndView modelAndView = new ModelAndView(new View() {
			...
			public void render(Map<String, ?> model, HttpServletRequest request, HttpServletResponse response) throws Exception {
				File file = streamingTaskManager.startMediaItemDownload(mediaItemId);		
				processRequest(request, response, file, content);
...
//				outputStream.close();
	 ...
	 */
	private void processRequest(HttpServletRequest request, HttpServletResponse response, long mediaItemId, boolean content) throws IOException {
