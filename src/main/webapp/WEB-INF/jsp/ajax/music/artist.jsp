...
<ul class="control-menu" id="artist-id-<c:out value="${artistPage.artist.id}" />">
	<li class="first"><a href="javascript:;" id="play-all"> <spring:message code="action.play-all" />
	</a></li>
...
<ul id="albums" class="items">
	<c:forEach items="${artistPage.artist.albums}" var="album">
		<li id="album-id-${album.id}" class="item"><a class="album-cover" href="javascript:;" rel="address:/address-load-album-${album.id}"> <img
				src="<c:url value="/app/music/album-art-thumbnail/${album.id}" />" title="<c:out value="${album.artist.name}" /> - <c:out value="${album.name}" />"
