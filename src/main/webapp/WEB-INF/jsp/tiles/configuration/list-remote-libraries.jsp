...
<script type="text/javascript">
	...
	$("#new-remote-library-container input[type=button]").click(function() {
	    ...
	    window.location.href = "<c:url value="/configuration/new-remote-library" />?remoteLibraryUrl=" + remoteLibraryUrl;
	});
    });
</script>
...
<div class="sub-panel">
	...
	<ul class="main-menu">
		<c:forEach items="${listRemoteLibrariesPage.remoteLibraries}" var="library">
			<li><a href="<c:url value="/configuration/edit-remote-library?libraryId=${library.id}" />"><c:out value="${library.name}" /></a></li>
		</c:forEach>
	</ul>
