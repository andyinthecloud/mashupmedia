...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("h1.edit").editable("<c:url value="/restful/media/music/save-album-name" />", {
        ...
        $.getJSON("<c:url value="/remote/music/artist/${albumPage.album.artist.id}" />", function(data) {
            displayRemoteArtistInformation(data);
        });

        $("#re-encode").click(function() {            
            $.post("<c:url value="/restful/encode/music-album" />", {
...
<div class="title-with-player-control">
	...
	<h2>
		- <a rel="internal"
			href="<c:url value="/music/artist/${albumPage.album.artist.id}" />">${albumPage.album.artist.name}</a>
	</h2>
...
<div class="album">
	...
	<div class="album-art">
		<img
			src="<c:url value="/music/album-art/original/${albumPage.album.id}" />"
