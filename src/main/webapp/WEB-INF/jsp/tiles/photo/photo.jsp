...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $("div.dynamic-content div.photo").on("swipeleft", function(event, ui) {
            loadInternalPage("${photo.nextPhoto.displayTitle}", "<c:url value="/photo/show/${photo.nextPhoto.id}?sequenceType=${photoPage.mediaItemSequenceType}" />");
        });
        
        $("div.dynamic-content div.photo").on("swiperight", function(event, ui) {
            loadInternalPage("${photo.previousPhoto.displayTitle}", "<c:url value="/photo/show/${photo.previousPhoto.id}?sequenceType=${photoPage.mediaItemSequenceType}" />");
        });
    });
...
<c:if test="${not empty photo.previousPhoto.id}">
	<div id="photo-previous">
		<a rel="internal" title="${photo.previousPhoto.displayTitle}"
			href="<c:url value="/photo/show/${photo.previousPhoto.id}?sequenceType=${photoPage.mediaItemSequenceType}" />"
...
<c:if test="${not empty photo.nextPhoto.id}">
	<div id="photo-next">
		<a rel="internal" title="${photo.nextPhoto.displayTitle}"
			href="<c:url value="/photo/show/${photo.nextPhoto.id}?sequenceType=${photoPage.mediaItemSequenceType}" />"
