...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("h1.edit").editable("<c:url value="/restful/media/music/save-artist-name" />", {
		...
		$.getJSON("<c:url value="/remote/music/artist/${artistPage.artist.id}" />", function( data ) {
			displayRemoteArtistInformation(data);			
		});	

        $("div.dynamic-content div.title-with-player-control div.re-encode a").click(function() {
            $.post("<c:url value="/restful/encode/music-artist" />", { id: <c:out value="${artistPage.artist.id}" /> })
            .done(function( data ) {
