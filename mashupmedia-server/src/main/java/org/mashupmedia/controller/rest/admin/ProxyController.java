...
public class ProxyController {
    ...
    public ProxyPayload getProxy() {
        return getProxyPayload();
    }

    @PutMapping(name = "/", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseStatus(HttpStatus.OK)
    public ProxyPayload saveProxy(@RequestBody ProxyPayload proxyDTO) {
        configurationManager.saveConfiguration(MashUpMediaConstants.PROXY_ENABLED, BooleanUtils.toStringTrueFalse(proxyDTO.getEnabled()));
